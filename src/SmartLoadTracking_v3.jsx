    ...
    try {
      if (mode === "login") {
        ...
        if (data.user) {
          ...
          // Sign in immediately so we have auth context to save profile
          const { data: signInData } = await supabase.auth.signInWithPassword({ email: email.trim(), password: pass });
          // Save profile to Supabase WITH invite_code
          const { error: profileError } = await supabase.from("profiles").upsert(
            { id: uid, name: fullName.trim(), role, owner_uid: ownerUid || uid, plan: "free", invite_code: inviteCode },
            { onConflict: "id" }
          );
...
function DriversTab({ session, loads, rates }) {
  ...
  // ── Sync invite code to Supabase on mount so it works cross-browser ──
  useEffect(()=>{
    const u=getUsers();
    const code=u[session.uid]?.inviteCode;
    if(code && session.uid){
      supabase.from("profiles")
        .upsert({ id: session.uid, name: session.fullName||session.name, role:"owner", owner_uid: session.uid, plan:"free", invite_code: code }, { onConflict:"id" })
        .then(({error})=>{ if(error) console.error("invite sync error:",error); });
    }
  },[session.uid]);
  ...
  const regen=()=>{
    ...
    u[session.uid].inviteCode=newCode;
    saveUsers(u);
    setUsersState({...u});
    // Also update Supabase so new code works cross-browser
    supabase.from("profiles")
      .upsert({ id: session.uid, name: session.fullName||session.name, role:"owner", owner_uid: session.uid, plan:"free", invite_code: newCode }, { onConflict:"id" })
      .then(({error})=>{ if(error) console.error("regen sync error:",error); });
  };
...
export default function SmartLoadTracking() {
  ...
  return (
    <div style={{ fontFamily: "'Mulish',sans-serif", minHeight: "100vh", width: "100%", maxWidth: "100%", overflowX: "hidden" }}>
      ...
      {/* ── New Premium tabs ── */}
      {tab === "ifta"       && isOwner && <IFTATab session={session} loads={visibleLoads} />}
